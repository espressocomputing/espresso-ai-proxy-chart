...
- Report version info as OTEL resource attributes: `service_version` (proxy image version,
  derived from `image.tag`) and `chart_version` (this chart's version, from `.Chart.Version`).
